...
argparser.add_argument('--save_path', type=str, default='./Results_33_gauss_collecting/')
argparser.add_argument('--load_path', type=str, default='./Results_33_gauss_mode')
argparser.add_argument('--load_num', type=int, default=11)
...
argparser.add_argument('--iter_num', type=int, default=20) #new added
...
for i in range(pa_number):
    for j in range(po_number):
        ...
        pa_payoff[i, j], po_payoff[i, j], _ = simulate_payoff(patrollers, poachers, i, j, env_pa, sess, 
            args, pa_type = pa_type[i], po_type = po_type[j])
