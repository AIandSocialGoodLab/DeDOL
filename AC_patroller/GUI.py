...
def test_gui(poacher, patroller, sess, args, pa_type, po_type):
    ...
    def run_step():
        ...
        if pa_type == 'DQN':
            ...
            state = np.array([pa_state])
            pa_action = patroller.infer_action(sess=sess, states=state)
        ...
        if end_game or (t == args.max_time - 1):
            ...
            if e == args.gui_test_num:
                # print(np.mean(pa_total_reward), np.mean(po_total_reward), np.mean(game_len))
                master.destroy()
...
if __name__ == '__main__':
    ...
    patroller = PatrollerPolicy(args, "pa_policy")

    args.column_num = args.row_num
    ...
    pa_total_reward, po_total_reward, length = test_gui(poacher, patroller, sess, args, pa_type = 'PG', po_type = 'PARAM')
    print(pa_total_reward)