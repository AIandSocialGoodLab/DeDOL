...
class Env(object):
    ...
    # used to train a DQN against a cfr-based agent
    def cfrstep(self, pa_action, po_action, snare_flag, train = True, mode = None):
        '''
        The interface between the DQN agent and cfr agent
        mode: cfr_pa or cfr_po, specifying the cfr agent
        '''
        # Place the snare by the poacher
        if snare_flag and self.poacher_snare_num > 0:
            self.place_snare(self.po_loc)
        ...
        # this process includes remove snares and kill animals
        if train:
            pa_reward, po_reward, pa_remove_snare = self.get_reward_train(po_ori_loc, po_new_loc, 
                                                        pa_ori_loc, pa_new_loc, cfr = True) 
        else:
            pa_reward, po_reward, pa_remove_snare = self.get_reward_test(cfr = True)
        ...
        # training a po DQN against a cfr-based pa agent
        if mode == 'cfr_pa':
            return self.get_po_state(), po_reward, self.end_game, pa_ob_footprint, pa_remove_snare
        elif mode == 'cfr_po': # train a pa DQN against a cfr-based po agent
            return self.get_pa_state(), pa_reward, self.end_game, po_ob_footprint 
        elif mode == 'cfr_both':
            return pa_reward, po_reward, self.end_game, pa_ob_footprint, po_ob_footprint, pa_remove_snare
    ...
    def envstep(self):
        '''
        After poacher and patroller have moved, remove snares and set catch flags.
        '''
        pre_snare_state = self.snare_state.copy()
        ...
        # only compute this reward if the poacher is caught for the first time 
        ...
        return pa_reward, -pa_reward, remove_cnt, pre


    def chancestep(self, number):
        # kill animals
        pre_snare_state = self.snare_state.copy()

        pa_reward, po_reward = 0, 0
        ...
        return pa_reward, po_reward, pre_snare_state
    ...
    def chanceundo(self, pre_snare_state):
        self.snare_state = pre_snare_state
