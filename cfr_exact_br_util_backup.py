        ...
        if player == 'c':
            if action_history == '': # reset game
                ...
                for idx in range(len(chance_actions)):
                    ...
                    self.build_trees(action_history_, p1info, p2info_, 1, probs[idx], 0, 0, mode)
                return
                # else:
                #     self.game.reset_game(self.args.po_location)
                #     action_history_ = action_history + str(self.args.po_location)
            ...
            u1, u2, remove_cnt, envpre = self.game.envstep()
            ...
            for idx in chance_actions:
                u1_, u2_, chancepre = self.game.chancestep(number = chance_actions[idx])
                p1info_ = p1info
                p2info_ = p2info
                ...
                if self.game.home_flag: # poacher returned home 
                    p2info_ += '&'
                if remove_cnt > 0: # patroller removed snares
                    p1info_ += '#' + str(remove_cnt)
                chance_prob_ = chance_prob * probs[idx]
                action_history_ = action_history + str(chance_actions[idx])
                # action_history_ = action_history + str(idx)
                self.build_trees(action_history_, p1info_, p2info_, 1, chance_prob_, p1u + u1 + u1_, p2u + u2 + u2_, mode)
                self.game.chanceundo(chancepre)

            self.game.envundo(envpre)
            return 
